      ...
      steps {
        script {
          ...
          withCredentials([usernamePassword(credentialsId: 'Github', usernameVariable: 'GITHUB_USERNAME', passwordVariable: 'GITHUB_PASSWORD')]) {
              echo "Discovery"
              def repoURL = """
                https://portainer.turkiyeoku.com/api/endpoints/1/docker/build?t=ms-discovery:v1&remote=https://$GITHUB_USERNAME:$GITHUB_PASSWORD@github.com/$GITHUB_USERNAME/okta-spring-microservices-docker-example.git&dockerfile=discovery/Dockerfile&nocache=true
              """
      ...
      steps {
        script {
          ...
          withCredentials([usernamePassword(credentialsId: 'Github', usernameVariable: 'GITHUB_USERNAME', passwordVariable: 'GITHUB_PASSWORD')]) {
            ...
            createStackJson = """
              {"Name": "MS", "SwarmID": "$swarmInfo.ID", "RepositoryURL": "https://github.com/$GITHUB_USERNAME/okta-spring-microservices-docker-example", "ComposeFilePathInRepository": "docker-compose.yml", "RepositoryAuthentication": true, "RepositoryUsername": "$GITHUB_USERNAME", "RepositoryPassword": "$GITHUB_PASSWORD"}
            """
